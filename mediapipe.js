...
export class MediaPipeTracker {
  ...
  _latchHandedness(key, landmarks, observed) {
    ...
    // Before the vote settles, report the running majority so the ring is oriented sensibly from the
    return majority;
  }

  // Only ever reached when the detector DID return hands
  ...
  _send(key, image) {
    ...
    return new Promise((resolve) => {
      // Release any waiter left over from a previous frame before taking the slot
      this._resolve(key);
      this._resolvers[key] = resolve;
      try {
        const p = solution.send({ image });
        if (p && typeof p.then === 'function') {
          // onResults normally settles this first
  ...
  async processFrame(video) {
    ...
    try {
      // The first frame also pays for downloading the models, which on a phone can take a while
